...
export default function App() {
  ...
  if (screen === "business-login") return <BusinessPortal businesses={businesses} packages={packages} onBack={() => setScreen("landing")} />;
  ...
  const tabs = [
    ...
    { id: "packages",   label: "📦 Log Package" },
    { id: "pickup",     label: "✅ Pickup" },
    { id: "businesses", label: "🏢 Businesses" },
    { id: "payments",   label: "💵 Payments" },
    { id: "reports",    label: "📈 Reports" },
    ...(currentUser?.role === "admin" ? [{ id: "staff", label: "👥 Staff" }] : []),
  ];

  const ctx = { packages, businesses, savePackages, saveBusinesses, showToast, showConfirm, setTab, pkgCounter, staffAccounts, saveStaff, currentUser };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "'Inter',system-ui,sans-serif" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 24px" }}>
        ...
        {tab === "businesses" && <Businesses {...ctx} />}
        {tab === "payments"   && <Payments {...ctx} />}
        {tab === "reports"    && <Reports {...ctx} />}
        {tab === "staff"      && currentUser?.role === "admin" && <StaffManager {...ctx} />}
      </div>
    </div>
  );
}
...
function LogPackage({ packages, businesses, savePackages, showToast, pkgCounter }) {
  const empty = { recipientName: "", recipientPhone: "", recipientEmail: "", businessName: "", trackingNumber: "", description: "", fee: "5.00", notes: "" };
      ...
      <div style={{ background: "#fff", borderRadius: 14, padding: 24, boxShadow: "0 1px 3px rgba(0,0,0,.07)" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          ...
          <div>
            ...
            <input style={inputStyle} value={form.trackingNumber} onChange={e => set("trackingNumber", e.target.value)} placeholder="1Z999AA10123456784" />
          </div>
          <div>
            <label style={labelStyle}>Pickup Fee (KSH)</label>
            <input style={inputStyle} type="number" step="0.50" value={form.fee} onChange={e => set("fee", e.target.value)} />
          </div>
...
function Pickup({ packages, savePackages, showToast }) {
  ...
  const defaultMsg = (pkg) =>
    `Hello ${pkg.recipientName}, your package ${pkg.pkgLabel || ""} has arrived at DropPoint Nairobi and is ready for pickup. Please collect it at your earliest convenience. Thank you!`;
...
function Payments({ packages, savePackages, showToast, showConfirm }) {
  ...
  const markPaid = (id) => {
    savePackages(packages.map(p => p.id === id ? { ...p, paid: true, paidAt: Date.now() } : p));
    showToast("Payment recorded ✓", "success");
  };

  const markAllPaid = () => {
    showConfirm(`Mark all ${unpaid.length} pending packages as paid (KSH ${totalOwed.toFixed(2)})?`, () => {
  ...
  return (
    <div>
      <h2 style={{ margin: "0 0 20px", fontSize: 20, fontWeight: 800, color: "#1e293b" }}>Payments</h2>
      ...
      {unpaid.length > 0 && (
        <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 1px 3px rgba(0,0,0,.07)", overflow: "hidden", marginBottom: 24 }}>
          <div style={{ padding: "14px 20px", borderBottom: "1px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span style={{ fontWeight: 700, fontSize: 15, color: "#1e293b" }}>Pending Payment</span>
            ...
            <tbody>
              {unpaid.map((p, i) => (
                <tr key={p.id} style={{ borderTop: i === 0 ? "none" : "1px solid #f1f5f9" }}>
                  ...
                  <td style={{ padding: "11px 14px" }}>
                    <button onClick={() => markPaid(p.id)} style={btnStyle("#ecfdf5", "#10b981", { fontSize: 12 })}>Mark Paid</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {paid.length > 0 && (
        <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 1px 3px rgba(0,0,0,.07)", overflow: "hidden" }}>
          <div style={{ padding: "14px 20px", borderBottom: "1px solid #f1f5f9" }}>
            <span style={{ fontWeight: 700, fontSize: 15, color: "#1e293b" }}>Collected ({paid.length})</span>
          </div>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead><tr style={{ background: "#f8fafc" }}>
...
function BusinessPortal({ businesses, packages, onBack }) {
  ...
  const pending = myPkgs.filter(p => p.status !== "picked_up");
    ...
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "'Inter',system-ui,sans-serif" }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "28px 24px" }}>
        ...
        <div style={{ display: "flex", gap: 14, marginBottom: 28, flexWrap: "wrap" }}>
          {[
            { label: "Total Dropped Off", value: myPkgs.length,    color: "#6366f1", bg: "#ede9fe" },
            { label: "Pending Pickup",    value: pending.length,   color: "#f59e0b", bg: "#fef3c7" },
        ...
        {pending.length > 0 && (
          <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 1px 3px rgba(0,0,0,.07)", overflow: "hidden", marginBottom: 24 }}>
            <div style={{ padding: "14px 20px", borderBottom: "1px solid #f1f5f9", background: "#fef3c7", display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ fontWeight: 700, fontSize: 15, color: "#92400e" }}>Awaiting Pickup ({pending.length})</span>
            </div>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead><tr style={{ background: "#f8fafc" }}>
                {["Pkg #", "Recipient", "Dropped Off", "Status"].map(h => (
                  <th key={h} style={{ padding: "10px 16px", textAlign: "left", fontSize: 11, fontWeight: 700, color: "#94a3b8", letterSpacing: "0.06em", textTransform: "uppercase" }}>{h}</th>
                ))}
              </tr></thead>
              <tbody>
