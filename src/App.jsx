...
function CustomerLoginScreen({ packages, savePackages, onBack }) {
  ...
  const verify = () => {
    const id = pkgId.trim().toUpperCase().replace(/^#/, "");
    const c = contact.trim().toLowerCase();
    if (!id || !c) return setError("Please enter both your package ID and phone/email.");

    const pkg = packages.find(p => {
      const label = (p.pkgLabel || p.id).toUpperCase().replace(/^#/, "");
      const phone = (p.recipientPhone || "").replace(/\D/g, "");
      const email = (p.recipientEmail || "").toLowerCase();
      const contactDigits = contact.replace(/\D/g, "");
      return label === id && (
        (contactDigits.length >= 9 && phone.includes(contactDigits.slice(-9))) ||
        email === c
      );
    });

    if (pkg) {
      // Show all packages for this contact
      const allPkgs = packages.filter(p => {
        const phone = (p.recipientPhone || "").replace(/\D/g, "");
        const email = (p.recipientEmail || "").toLowerCase();
        const contactDigits = contact.replace(/\D/g, "");
        return (contactDigits.length >= 9 && phone.includes(contactDigits.slice(-9))) || email === c;
      });
      setCustomerPkgs(allPkgs);
      setVerified(true);
      setError("");
    } else {
      setError("No package found. Please check your package ID and phone/email.");
    }
  };
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #1e293b 0%, #0f4c81 100%)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Inter',system-ui,sans-serif", padding: 24 }}>
      <div style={{ background: "#fff", borderRadius: 20, padding: 32, width: "100%", maxWidth: 380, boxShadow: "0 20px 60px rgba(0,0,0,.3)" }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <div style={{ fontSize: 13, color: "#64748b", marginTop: 4 }}>Enter your package ID and phone number or email</div>
        </div>
        <div style={{ display: "grid", gap: 14, marginBottom: 14 }}>
          <div>
            <label style={labelStyle}>Package ID</label>
            <input style={{ ...inputStyle, fontFamily: "monospace", fontWeight: 700, fontSize: 16, letterSpacing: 2, textTransform: "uppercase" }}
              value={pkgId} onChange={e => setPkgId(e.target.value.toUpperCase())}
        ...
        {error && <div style={{ background: "#fef2f2", color: "#ef4444", fontSize: 13, padding: "10px 14px", borderRadius: 8, marginBottom: 14, fontWeight: 600 }}>{error}</div>}
        <button onClick={verify} style={{ width: "100%", background: "#25D366", color: "#fff", border: "none", borderRadius: 10, padding: "13px", fontSize: 15, fontWeight: 700, cursor: "pointer" }}>
          Track Package
        </button>
...
function Pickup({ packages, savePackages, showToast }) {
  ...
  const openSMS = async (pkg) => {
    const phone = "+" + fmtPhone(pkg.recipientPhone);
    const message = defaultMsg(pkg);
    showToast(`Sending SMS to ${pkg.recipientName}...`, "info");
    try {
      ...
      if (data.success) {
        ...
        savePackages(updated);
        showToast(`SMS sent to ${pkg.recipientName} ✓`, "success");
      } else {
        showToast("SMS failed — check Africa's Talking account", "error");
      }
    } catch(e) {
      showToast("SMS failed — check your connection", "error");
    }
  };
                  ...
                  {(p.status === "waiting" || p.status === "notified") && (
                    <>
                      ...
                      <button onClick={() => openSMS(p)}
                        style={btnStyle("#0ea5e9", "#fff", { fontSize: 13 })}>
                        💬 SMS
                      </button>
                    </>
                  )}
